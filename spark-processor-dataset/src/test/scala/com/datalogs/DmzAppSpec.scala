...
import com.datalogs.dataschema.{MetaInfo, SampleDataset}
import com.datalogs.dataset.SampleDatasetEnricher
...
class DmzAppSpec extends FlatSpec with Workflow with MustMatchers {
  "DmzApp" should "partition evenly regardless of skew" in {
    ...
    val recs = List.fill(100) {
      ...
      RichRecord(new SampleDataset(0, "", "", "", "", 0, 0, 0, dt.toString(), new MetaInfo(0L,"","","", 0L, 0)), DateTimeContext(dt))
    } ++ List.fill(400) {
  ...
  it should "Trap records outside interval during enrich" in {
    ...
    val recs : List[Either[ProcessingFailure, PartialRecord[SampleDataset]]] = (0 until 5).map(ms => {
      val pr1 = new PartialRecord[SampleDataset](new SampleDataset(0, "", "", "", "", 0, 0, 0, (new DateTime(2018, 10, 1, 0, 0, 0, ms)).toString(), new MetaInfo(0L,"","","", 0L, 0)),"", "", "", Map.empty[String, String])
      Right(pr1)
    }).toList
    ...
    val results = sc.parallelize(recs).enrichWith(SampleDatasetEnricher, interval).collect()
